...
// ── Commands (service-oriented, via Token Vault) ──────────────
...
const SLASH_COMMANDS: SlashCommand[] = [
  { command: "/send", label: "Send", description: "Send email or message via connected service", args: "<service> <message>", icon: <Mail className="h-3.5 w-3.5" /> },
  { command: "/pay", label: "Pay", description: "Process a payment via Stripe, PayPal, or Klarna", args: "<amount> <to>", icon: <CreditCard className="h-3.5 w-3.5" /> },
  { command: "/post", label: "Post", description: "Publish content to a social platform", args: "<platform> <content>", icon: <Share2 className="h-3.5 w-3.5" /> },
  { command: "/schedule", label: "Schedule", description: "Schedule a meeting or event via Google Calendar", args: "<title> <time>", icon: <Calendar className="h-3.5 w-3.5" /> },
  { command: "/connect", label: "Connect", description: "Connect a new service through Token Vault", args: "<service>", icon: <Link2 className="h-3.5 w-3.5" /> },
  { command: "/permissions", label: "Permissions", description: "View or modify this agent's service scopes", args: "[service]", icon: <ShieldCheck className="h-3.5 w-3.5" /> },
  { command: "/approve", label: "Approve", description: "Approve a pending CIBA authorization request", args: "[request-id]", icon: <CheckCircle className="h-3.5 w-3.5" /> },
  { command: "/revoke", label: "Revoke", description: "Revoke agent's access to a connected service", args: "<service>", icon: <Lock className="h-3.5 w-3.5" /> },
  { command: "/status", label: "Status", description: "Check agent's current task and service health", icon: <Eye className="h-3.5 w-3.5" /> },
  { command: "/escalate", label: "Escalate", description: "Escalate current task to human supervisor", icon: <AlertTriangle className="h-3.5 w-3.5" /> },
  { command: "/trust", label: "Trust", description: "View or request trust level change", args: "[level]", icon: <TrendingUp className="h-3.5 w-3.5" /> },
  { command: "/audit", label: "Audit", description: "View recent actions and Token Vault activity", args: "[n]", icon: <Shield className="h-3.5 w-3.5" /> },
];
...
function SlashCommandPalette({ query, onSelect }: { query: string; onSelect: (cmd: string) => void }) {
  const search = query.slice(1).toLowerCase();
  const filtered = SLASH_COMMANDS.filter((c) => c.command.toLowerCase().includes(search) || c.label.toLowerCase().includes(search));
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 8 }} className="absolute bottom-full left-0 right-0 mb-2 rounded-xl border border-border/50 dark:border-white/[0.1] bg-background/95 backdrop-blur-xl shadow-xl overflow-hidden z-50">
      <div className="px-3 py-2 border-b border-border/30 dark:border-white/[0.06]"><p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground/50">Commands</p></div>
      <div className="max-h-[240px] overflow-y-auto py-1">
        {filtered.map((cmd) => (
          <button key={cmd.command} onClick={() => onSelect(cmd.command + " ")} className="w-full flex items-center gap-3 px-3 py-2 hover:bg-muted/40 dark:hover:bg-white/[0.06] transition-colors text-left">
            <div className="h-7 w-7 rounded-lg bg-muted/40 dark:bg-white/[0.06] flex items-center justify-center text-muted-foreground/60 shrink-0">{cmd.icon}</div>
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2"><span className="text-[13px] font-medium text-foreground">{cmd.command}</span>{cmd.args && <span className="text-[11px] text-muted-foreground/40 font-mono">{cmd.args}</span>}</div>
    ...
    <div className="flex h-full">
      ...
      <div className="w-[340px] shrink-0 border-r border-border/40 dark:border-white/[0.08] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto px-2 space-y-0.5">
          {filtered.map((conv) => (
            <button key={conv.id} onClick={() => setSelectedId(conv.id)} className={cn("w-full flex items-start gap-3 p-3 rounded-xl transition-all duration-200 text-left", selectedId === conv.id ? "bg-primary/[0.08] dark:bg-white/[0.08]" : "hover:bg-muted/40 dark:hover:bg-white/[0.04]")}>
      ...
      <div className="flex-1 flex flex-col min-w-0">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 text-[11px] text-muted-foreground/60">
                <span>{selected.agentRole}</span><span className="text-muted-foreground/20">|</span><span>{TRUST_LABEL[selected.trustLevel]}</span><span className="text-muted-foreground/20">|</span><span className="capitalize">{selected.status}</span>
              </div>
            </div>
          </div>
          <div className="flex items-center gap-1">
            <button className="h-8 w-8 rounded-lg flex items-center justify-center text-muted-foreground/60 hover:text-foreground hover:bg-muted/40 dark:hover:bg-white/[0.06] transition-colors"><Pin className="h-4 w-4" /></button>
            <button className="h-8 w-8 rounded-lg flex items-center justify-center text-muted-foreground/60 hover:text-foreground hover:bg-muted/40 dark:hover:bg-white/[0.06] transition-colors"><MoreHorizontal className="h-4 w-4" /></button>
          </div>
        </div>
        ...
        <div className="p-4 border-t border-border/40 dark:border-white/[0.08]">
          <div className="relative">
            <AnimatePresence>{showCommands && <SlashCommandPalette query={messageInput} onSelect={handleCommandSelect} />}</AnimatePresence>
